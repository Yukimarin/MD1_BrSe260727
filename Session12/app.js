...
//While hay do...while: Khong biet truoc so lan lap, vong lap for biet truoc so lan lap
// VD: Cho 1 so ban dau, Viet chuong trinh cho nguoi dung nhap vao 1 so bat ky
// Neu so do lon hon so ban dau => In ra lon hon
// Neu so do nho hon so ban dau => In ra nho hon
// So do = so ban dau => Bingo va ket thuc chuong trinh
// MO hinh Input - Output - Process
// Input (Dau vao cua bai toan): Số ban đầu
// Process:
// B1: Khơi tao so ban dau
let guessNumber = 20;

let check = true;
// B4: Sử dụng vòng lặp. Vòng lặp chỉ kết thúc khi dự đoán đúng
while (true) {
  // B2: Khoi tao bien luu gia tri ma nguoi dung nhap
  let inputNumber = Number(prompt("Nhap vao so ban muon doan"));
  //   console.log(guessNumber, inputNumber);
  // B3: Su dung menh de dieu kien if.else de so sanh
  if (inputNumber > guessNumber) {
    console.log("So ban nhap lon hon so can doan");
  } else if (inputNumber === guessNumber) {
    console.log("BINGOOOOO!");
    break;// check = false 
  } else {
    console.log("So ban nhap nho hon so can doan");
  }
}
// console.log(guessNumber);
// Output (Dau ra cua bai toan):
// Neu so do lon hon so ban dau => In ra lon hon
// Neu so do nho hon so ban dau => In ra nho hon
// So do = so ban dau => Bingo va ket thuc chuong trinh
